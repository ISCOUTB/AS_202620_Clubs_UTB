...
## 4. Contrato asíncrono (notificaciones) — AsyncAPI 3.0

Justificado en el [ADR 0002](../adr/0002-integracion-sincrona-asincrona.md):
crear una publicación es síncrono (el club necesita confirmación inmediata);
avisarle a los interesados de que existe es asíncrono (nadie debería tener que
esperar en pantalla a que se les notifique a otros).

```yaml
asyncapi: 3.0.0
info:
  title: LinkClub — Notificaciones de Publicaciones
  version: 1.0.0
  description: >
    Contrato del flujo ASÍNCRONO de LinkClub: cuando se crea una publicación
    (POST /clubes/{club_id}/publicaciones, ver sección 3), el backend emite un
    evento de dominio que uno o más consumidores (servicio de notificaciones
    push, futura vista de "novedades" en tiempo real, etc.) procesan de forma
    independiente y desacoplada en el tiempo.
externalDocs:
  url: https://github.com/ISCOUTB/AS_202620_Clubs_UTB/blob/master/docs/adr/0002-integracion-sincrona-asincrona.md

defaultContentType: application/json

channels:
  publicacionCreada:
    address: linkclub.publicaciones.creada
    description: >
      Canal donde se publica un evento cada vez que un club crea una
      publicación nueva (aviso, encuesta o noticia), inmediatamente después de
      que la operación síncrona de creación fue confirmada al cliente HTTP.
    messages:
      publicacionCreadaEvent:
        $ref: "#/components/messages/PublicacionCreada"

operations:
  publicarEventoPublicacionCreada:
    action: send
    summary: El backend publica el evento cuando confirma una nueva publicación.
    channel:
      $ref: "#/channels/publicacionCreada"
    messages:
      - $ref: "#/channels/publicacionCreada/messages/publicacionCreadaEvent"

  consumirEventoPublicacionCreada:
    action: receive
    summary: >
      Un servicio consumidor (ej. notificador push) recibe el evento para
      avisar a los miembros e interesados del club. Puede haber cero, uno o
      varios consumidores; su disponibilidad no bloquea la creación de la
      publicación.
    channel:
      $ref: "#/channels/publicacionCreada"
    messages:
...
components:
  messages:
    PublicacionCreada:
      ...
      summary: Se emitió una nueva publicación (aviso, encuesta o noticia) en un club.
      contentType: application/json
      payload:
        $ref: "#/components/schemas/PublicacionCreadaPayload"

  schemas:
    PublicacionCreadaPayload:
      type: object
      required: [evento, version, publicacion_id, club_id, tipo, titulo, emitido_en]
      properties:
        evento:
          type: string
          const: "publicacion.creada"
        version:
          type: string
          description: Versión del esquema del evento (independiente de la versión de este archivo).
          example: "1.0"
        publicacion_id:
          type: string
          format: uuid
...
## 5. Estado de implementación (a la fecha de este contrato)
...
| Evento asíncrono `publicacion.creada` | ✅ | ❌ pendiente (no hay emisor ni consumidor todavía) |
